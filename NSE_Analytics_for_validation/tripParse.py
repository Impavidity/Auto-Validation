...
#            co2=0
    ...
    def segFind(df, trip_return, mode_thresh=100, isAM = True, dist_lim = 45.3, max_mode=5, max_walk=4.0):
    ...
    def tripFind(homeLoc,schoolLoc,day_frame,predicted_modes,mode_thresh=100):
        """
        ...
        if not home_sch_set.empty and homeLoc[0] is not None and schoolLoc[0] is not None:
            ...
            home_sch_dist = great_circle_dist(homeLoc, schoolLoc, unit="meters")
#            logging.warning(home_sch_dist)
#            logging.warning(home_idx)
#            logging.warning(school_idx)

            if home_idx['idx']<school_idx['idx'] and (home_idx['dist']<home_sch_dist*0.6 or home_sch_dist<1000):
...
#            logging.warning(home_idx)
        ...
        if ((trip_return['am_mode']==[3] and trip_return['pm_mode']==[]) or
            (trip_return['am_mode']==[] and trip_return['pm_mode']==[3]) or
            (trip_return['am_mode']==[3] and trip_return['pm_mode']==[3])):
            trip_return['travel_co2']+= 0.00001
        else:
    ...
    logging.debug("Length of POIs FOUND: " + str(len(pois)))
...
#        daily_trips.pop('pm_mode',None)
    ...
    # save pois in a proper format and return
    daily_pois = {'poi_lat': [],'poi_lon': []}
    for row in pois:
...
def trip_segment(data_frame, stopped_thresh=0.5, stopped_dwell=480):
    ...
    def store_poi(data_frame, idx_buffer, stop_time):
        ...
        pois['poi_latlon'].append([float(poi_lat),float(poi_lon)])
    ...
    round_decimal = 4;
    dist_2comb = 100; # m, distance close to which the two pois are combined
